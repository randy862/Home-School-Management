...
- Class conflict warnings and Performance Dashboard total averages are scoped to the active Academic Year so prior-year enrollments/grades do not leak into current-year views.
...
Next technical action is to sync the validated grade-level/reporting/course-grade-filter/student-schedule-grade-context/school-year-delete-cleanup/class-conflict-year-scope/performance-total-average-year-scope plus school-year scoped enrollment release to AWS, including tenant migrations `033` and `034`.
