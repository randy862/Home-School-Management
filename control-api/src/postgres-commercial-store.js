...
async function getCommercialOverviewBySubscriptionId(id) {
  ...
  const result = await pool.query(`
    SELECT
      ...
      account.owner_phone AS "ownerPhone",
...
async function updateCommercialSubscription(id, updates = {}) {
  ...
  const result = await pool.query(`
    ...
    SET
      ...
      included_billable_students = COALESCE($10, included_billable_students),
      per_student_overage_cents = COALESCE($11, per_student_overage_cents),
      current_billable_student_count = COALESCE($12, current_billable_student_count),
      current_overage_student_count = COALESCE($13, current_overage_student_count),
      last_billable_count_calculated_at = COALESCE($14, last_billable_count_calculated_at),
      updated_at = NOW()
    WHERE id = $1
    RETURNING
      id,
