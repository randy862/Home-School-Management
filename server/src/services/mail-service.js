    ...
    async sendTemplateEmail(input = {}) {
      ...
      if (!readiness.ready || typeof fetchImpl !== "function") {
        return buildResult({
          ...
          errorMessage: (readiness.ready ? ["Global fetch is required for outbound mail delivery."] : readiness.missing).join("; ")
        });
      }
...
function getReadiness(config = {}) {
  ...
  if (!["log_only", "allowlist_only", "real_send"].includes(mode)) {
    missing.push("MAIL_MODE must be one of log_only, allowlist_only, or real_send.");
  }
  if (!config.fromEmail) missing.push("MAIL_FROM_EMAIL is required.");
  if (provider !== "postmark") missing.push("MAIL_PROVIDER must be postmark.");
  if (mode !== "log_only" && !config.postmarkServerToken) {
    missing.push("MAIL_POSTMARK_SERVER_TOKEN is required for Postmark delivery.");
  }
  return {
